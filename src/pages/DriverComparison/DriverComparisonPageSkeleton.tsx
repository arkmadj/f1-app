...
interface DriverComparisonPageSkeletonProps {
...
function DriverComparisonPageSkeleton({
  selectedSeason,
}: DriverComparisonPageSkeletonProps): JSX.Element {
  ...
  return (
    ...
    >
      <span className="sr-only">
        {t("driverComparison.loading")} · {selectedSeason}
      </span>
      ...
      >
        ...
        <div className="mt-3 grid gap-5 min-[900px]:grid-cols-[1.2fr_0.8fr] min-[900px]:items-end">
          <div className="space-y-4">
            <SkeletonBlock className="h-14 w-[min(100%,26rem)] rounded-[1.8rem]" />
      ...
      <section className={`${cardBase} mt-6 p-5 min-[900px]:p-6`} aria-hidden="true">
        <div className="mb-5 flex flex-col gap-3 min-[720px]:flex-row min-[720px]:items-end min-[720px]:justify-between">
          <div className="space-y-3">
            <SkeletonBlock className="h-3 w-28 rounded-full" />
            <SkeletonBlock className="h-8 w-60 rounded-2xl" />
          </div>
...
export default DriverComparisonPageSkeleton;